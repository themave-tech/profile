...
addroute()
{
  if [ $HOSTNAME = $host1 ];
  then
  ip route add $ip0 dev bond0 proto kernel scope link src 192.168.137.105
  elif [ $HOSTNAME = $host2 ];
  then
  ip route add $ip0 dev bond1 proto kernel scope link src 192.168.137.105
  else
...
{
if [ $HOSTNAME = $host1 ];
ip link set dev bond0 address 3c:ec:ef:20:cd:50
elif [ $HOSTNAME = $host2 ];
then
ip link set dev bond1 address 3c:ec:ef:20:cd:80
else logWrite "ERROR. Unknown host"
exit 4
fi


  ip_check2=$(ip route | grep "$ip5" | awk '{print $9}')
  if  [[ ${ipcheck2} =~ ${ip5} ]]
  then
    delroute
  else
...
delroute()
{
  ip route del 192.168.137.0/24 dev bond1 proto kernel scope link src 192.168.137.105
  if [ $? -eq 0 ];
  then
    logWrite "OK"
  else
  ...
  ip_check3=$(ip addr | grep -i 'inet' | awk '{print $2}')
  if [[ ${ip_check1} =~ ${ip5} ]];
  then
  ipdel
  else
  logWrite "ERROR. File not found"
  exit 3
  fi

  ipdel()
{
  ip addr del 192.168.137.105/255.255.255.0 dev bond1:1
  if [ $? -eq 0 ]
  then
  logWrite "ERROR"
  exit 4
